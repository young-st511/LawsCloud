...
const outString = ['법', '등', '법률', '안', '지원', '특별법', '특례', '제한', '기본법','관리','진흥'];

const wordcloudData = {};

//# 실행 구역!!
for (const elem of agesAndYears) {
  wordcloudData[elem[0] + '-' + elem[1]] = makeBillsWordcloudData(
    elem[0],
    elem[1],
    100
  );
...
writeFileSync(
  `./wordcloud/cloudNouns/cloud-nouns-2.json`,
...
function makeBillsWordcloudData(age, year, wordsNum = 30) {
  const billsCloudData = makeWordcloudData(
    `./wordcloud/billsNouns/${age}th-${year}-nouns.txt`,
    wordsNum
  );
  // writeFileSync(
